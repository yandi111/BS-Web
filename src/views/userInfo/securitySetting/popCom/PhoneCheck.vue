<template>
 <div>
  <!-- 手机验证码  -->
  <div>
   <div class="ff0" style="font-size: 14px; margin-bottom: 9px; ">手机验证码</div>
   <div class="input-containerS" style="height: 42px;">
    ...
    <input style="height: 42px; padding-left: 12px; color: #F0F0F0" v-model="phoneCheckCode"
           maxlength="4" @focus="eventFocusS($event)" @blur="$event.target.style.border = 'none',
    ...
    <div class="input-icon " style="cursor: pointer;display: flex;">
     ...
     <div v-else @click="phondeVerifyCode" style="font-weight: 400; color: #90FF00; font-size: 12.5px;">
      获得验证码
     </div>
   ...
   <div @click="$refs.mobileCode.openDialog(getMethod)" style="color: #90FF00; font-size: 12px; margin-top: 7px; font-weight: 500;">
    未收到短信验证码？
   </div>
