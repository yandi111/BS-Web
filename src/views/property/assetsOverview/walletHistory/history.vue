    ...
    <div class="table" v-if="activeIndex === 5">
      <el-table :data="otcData" style="width: 100%">
        ...
        <el-table-column :label="$t('property.订单状态')">
          <template slot-scope="scope">
            <span v-if="scope.row.status === 0">{{$t('property.待支付')}}</span>
            <span v-if="scope.row.status === 1">{{$t('property.待放币')}}</span>
            <span v-if="scope.row.status === 2">{{$t('property.申诉中')}}</span>
            <span v-if="scope.row.status === 3">{{$t('property.已完成')}} </span>
            <span v-if="scope.row.status === 4">{{$t('property.取消')}}</span>
            <span v-if="scope.row.status === 5">{{$t('property.超时取消')}}</span>
            <span v-if="scope.row.status === 6">{{$t('property.申诉后交易最终完成')}}</span>
            <span v-if="scope.row.status === 7">{{$t('property.申诉后取消交易')}}</span>
            <span v-if="scope.row.status === 8">{{$t('property.用户违规取消')}}</span>
            <span v-if="scope.row.status === 9">{{$t('property.商户违规取消')}}</span>
          </template>
        </el-table-column>
    ...
    <div class="table" v-if="activeIndex === 7">
      <el-table :data="spotData" style="width: 100%">
        ...
        <el-table-column :label="$t('property.交易类型')">
          <template slot-scope="scope">
            <span v-if="scope.row.dealType === 1">{{$t('property.限价交易')}}</span>
            <span v-if="scope.row.dealType === 2">{{$t('property.市价交易')}}</span>
          </template>
        </el-table-column>
    ...
    <div class="table" v-if="activeIndex === 9">
      <el-table :data="agentData" style="width: 100%">
        ...
        <el-table-column :label="$t('property.金额类型')">
          <template slot-scope="scope">
            <span v-if="scope.row.amountType === 1">{{$t('property.可用金额')}}</span>
            <span v-if="scope.row.amountType === 2">{{$t('property.冻结金额')}}</span>
          </template>
        </el-table-column>
