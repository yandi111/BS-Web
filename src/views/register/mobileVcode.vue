  ...
  <div style=" background: #1B1B1B; border-radius: 10px;">
   <div style="padding: 40px; width: 480px;">
    ...
    <div class="input-container" style=" margin-top: 30px;">
     ...
     <input v-model="verifyCode" maxlength="4" @focus="eventFocus($event)"
            @blur="$event.target.style.border = 'none', eventFlag = false" class="custom-input"
            :placeholder="phoneEmail ? '请输入4位手机验证码' : '请输入4位邮箱验证码'" type="text"/>
    ...
    <div @click="verificationCode"
         style="margin-top: 10px;font-size: 14px; color: #90FF00;cursor: pointer;">
...
import {getBSEX, onVerifyRegister} from "@/api/login";
...
export default {
 ...
 methods: {
  ...mapMutations(['setToken', 'setAccountList']),
  ...
  registerBtn() {
   ...
   Promise.try(async () => {
    ...
    res.data.data.status === "AUTHENTICATED" && this.registerFn()
   })
  },
  ...
  accountShowList(res) {
   ...
   const exists = emailList.some(item => item.account === emailItem.account);
   if (!exists) emailList.unshift(emailItem); // 如果没有，则添加到数组开头
  ...
  registerFn() {
   ...
   Promise.try(() => {
    return getBSEX(authVerifyRequestVO)
   }).then(res => {
