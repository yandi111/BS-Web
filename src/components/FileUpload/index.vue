<template>
  <div class="upload-file" @click="$emit('handleUploadFile',fileList)">
    <el-upload
      multiple
    ...
    >
      <!-- 上传按钮 -->
      <el-button size="mini" type="primary">{{ uploadFileName }}</el-button>
      <!-- 上传提示 -->
...
export default {
  ...
  data() {
    return {
      ...
      uploadList: [],
      baseUrl: baseApi,
      uploadFileUrl: baseApi + "/user/file/upload-file", // 上传文件服务器地址
      headers: {
  ...
  methods: {
    ...
    handleExceed() {
      this.$message({
        message:`上传文件数量不能超过 ${this.limit} 个,请删除后重新上传!`,
            type:'error'
          });
    },
    // 上传失败
