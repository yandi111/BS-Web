<template>
  <div class="flash">
    ...
    <div class="content">
      <div class="container">
        <el-form label-position="top">
          ...
          <el-form-item>
            <div class="item-consume" @click.stop>
              <div class="pay-title">
                ...
                <div class="pay-all" @click.stop="handleAll">
                  {{ $t("property.全部") }}
                </div>
              </div>
              <div class="pay-box">
          ...
          <el-form-item>
            <div class="exchange" @click="onFlash">
              {{ $t("property.兑换") }}
            </div>
          </el-form-item>
        </el-form>
      ...
      <div>
        <div class="content-right">
          <div class="c-icon">
            <img src="@/assets/property-imgs/flash-banner.png" alt="" />
          </div>
          <div class="c-text">
            <div class="c-title">{{ $t("property.闪兑") }}</div>
            <div>
              {{
                $t(
                  "property.闪兑功能专为用户快捷兑换加密货币而设计，帮助用户在兑换加密货币时摆脱多次交易或在不同钱包和交易所之间频繁转移加密资产的烦恼。BSEX闪兑功能极大程度地简化了兑换流程，为用户提供了加密货币无缝兑换的捷径。"
                )
              }}
            </div>
          </div>
    ...
    >
      ...
      <template #dia_footer>
        <el-button class="modal-btn" @click="handleCancel">{{
          $t("property.取消")
        }}</el-button>
        <el-button
...
export default {
  ...
  data() {
    return {
      ...
      checkArr: [
        ...
        {
          id: 1,
          name: this.$t("property.现货"),
          flag: false,
        },
      ],
      fromType: 2, //需要兑换的类型 默认资金账户
      commission: "", //手续费
      // 闪兑精度
  ...
  methods: {
    ...
    onChange(val, type) {
      ...
      if (type === "get") {
        if (this.getValue === "") {
          ...
          this.payValue = (this.getValue / this.exchangeRate).toFixed(
            this.topDecimalPlaces
          );
        }
      } else {
        if (!this.payValue) {
          ...
          this.getValue = (this.payValue * this.exchangeRate).toFixed(
            this.bottomDecimalPlaces
          );
        }
      }
    },
    ...
    getFlagType() {
      this.fromType =
        Number(this.$route.query.fromAccountType) || this.fromType;
