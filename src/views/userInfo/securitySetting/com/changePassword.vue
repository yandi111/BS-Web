    ...
    <div class="dialog-content" style="border:1px solid #252525;">
     ...
     <div style="place-self: 30px; padding-right: 30px; padding-left: 30px; padding-top: 29px;">
      ...
      <div>
       ...
       <!-- 旧资金密码 -->
       <div style="margin-bottom: 9px; font-size: 14px;color: #F0F0F0; margin-top: 19px;">{{$t('旧资金密码')}}
       </div>
       <div class="input-container" style="margin-bottom: 19px;" OldFund
            :style="{ border: eventFlagOldFund ? '0.5px solid #90FF00' : '0.5px solid rgba(0,0,0,0)' }">
        <div class="inputLeft" style="height: 42px; padding-left: 12px;">
         <input v-model="passwordDataOldFund" @focus="eventFocusOldFund($event)" @blur="eventFlagOldFund = false" class="custom-input" :placeholder="$t('lang_2587')" :type="iconOpenOldFund == 1 ? 'password' : 'text'" style="color: #F0F0F0;border:none;letter-spacing: 2px;height: 42px;"/>
        </div>
       ...
       <!-- 资金密码 -->
       <div style="margin-bottom: 9px; font-size: 14px;color: #F0F0F0;">{{$t('lang_2787')}}</div>
       <div class="input-container"
        ...
        <div class="inputLeft" style="height: 42px; padding-left: 12px;">
         <input v-model="passwordData" maxlength="20" @focus="eventFocus($event)"
                @blur="eventFlag = false" class="custom-input" :placeholder="$t('lang_2815')"
      ...
      <div>
       ...
       <div v-if="EMAILshow" style="margin-top: 20px;">
        <EmailCheck @emailCheckCodeClick='emailCheckCodeClick' :bizId='bizId'
                    :method='EMAILmethod' :authBizEnum='authBizEnum'/>
       </div>
       <!-- 谷歌验证 -->
       <div v-if="GOOGLE_AUTHENTICATORshow" style=" margin-top: 29px;">
        ...
        <div class="input-container"
         ...
         <div class="inputLeft" style="height: 40px; width: 100%;">
          <input maxlength="4" v-model="googleCheckCode"
                 @focus="eventFocusGoogle($event)" @blur="eventFlagGoogle = false"
                 class="custom-input" :placeholder="$t('lang_1639')" type="text"
...
import {ModifyPass, onModifyPass} from "@/api/user";
   ...
   const params = {
    ...
    verifyInfos: [
...

     {
      ...
      verifyCode: SHA256(this.passwordDataOldFund).toString(),
     },
