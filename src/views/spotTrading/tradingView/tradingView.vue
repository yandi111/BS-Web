<template>
  <div class="trade-view" :class="theme === 'light' ? '' : 'activeBg'" ref="fullRef">
   <div class="trade-header" :class="theme === 'light' ? '' : 'activeBorder'">
    ...
    <div class="tabs-left" v-show="pType === 'k'">
     <div class="coin">
      <img class="avatar" :src="iconUrl" alt="" />
      {{ coinTitle }}
     </div>
     <div :class="[activeId === item.value ? 'times-active' : '', 'times']" v-for="item in tabsArr" :key="item.value" v-show="item?.active" @click="checkResolution(item)">
      {{ item.label | translate }}
     </div>
     <div class="time-select" @mouseenter="mouseenter" @mouseleave="mouseleave">
      <div class="time-box"></div>
      <div class="s-icon">
       <span :class="{'times-active': !tabsArrNewActive(activeId, true)?.active,}">
        {{ !tabsArrNewActive(activeId, true)?.active ? tabsArrNewActive(activeId, true)?.label : $t("lang_1868") | translate }}
       </span>
 ...
 import {GetHistory} from "@/api/spotTrading";
 ...
 export default {
  ...
  computed: {
   ...
   data() {
    return {
     topic: `trade-kline-${this.symbol}-${this.interval}`,
     symbol: this.symbol,
     count: 1000,
  ...
  mounted() {
   ...
   this.$EventBus.$on('getCoins', e => {
    this.iconUrl = e.logo
    this.coinTitle = e.coinTitle
    this.symbol = e.name;
 
    this.chart.setSymbol(this.symbol, this.interval.replace('MIN', ''), e => {
  ...
  methods: {
   ...
   checkResolution(item) {
    if (item.value === 0 && this.activeId !== 0) {
     this.chart.remove()
   ...
   chooseItem(item) {
    if (!this.iseDit) return this.checkResolution(item)
   ...
   initChars(style = 1) {
    ...
    this.chart = new widget({
     ...
     overrides: {
      ...
      "mainSeriesProperties.style": style, // 主系列样式
   ...
   async historyBsexMarket(
    ...
    // 请求历史K线数据
    Promise.try(async () => {
     return GetHistory({symbol: symbolInfo.name, count: 1000, to: rangeEndInfo, resolution: this.interval})
    }).then(res => {
     // 遍历数据并格式化成 TradingView 需要的格式
     klineData = res.data.map(item => {
      return {
       time: Number(item.timeStamp), // 注意时间戳转换为毫秒
       ...
       rate: item.fluctuation
      }
     })
 
     this.$EventBus.$emit('currencyInfo', klineData[klineData.length - 1])
