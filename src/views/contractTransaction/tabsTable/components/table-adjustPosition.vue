            ...
            <div class="item">
              ...
              <div class="describe">
                ...
                <p v-else>
                  {{
                    $t(
                      "contract.当最新价格到达x时，将会触发限价止盈委托平仓当前仓位。预计盈亏为xUSDT",
                      [profit ? profit : "0.0", profitvalue]
                    )
                  }}
                </p>
            ...
            <div class="item">
              ...
              <div class="describe">
                ...
                <p v-else>
                  {{
                    $t(
                      "contract.当最新价格到达x时，将会触发限价止损委托平仓当前仓位。预计盈亏为xUSDT",
                      [loss ? loss : "0.0", lossvalue]
                    )
                  }}
                </p>
...
export default {
  ...
  data() {
    return {
      ...
      profitvalue: "--",
      lossvalue: "--",
      coinValue: null, //币的输入值
      usdtValue: null, //usdt单位
    };
  },
  computed: {
    ...mapState({
    ...
    //   if (this.profit) {
    //     value =
    //       this.data.positionDirection == 1
    //         ? (this.profit * 1 - this.data.positionAveragePrice * 1) *
    //           this.data.faceValue *
    //           this.data.positionAmount
    //         : (this.data.positionAveragePrice * 1 - this.profit * 1) *
    //           this.data.faceValue *
    //           this.data.positionAmount;
    //   } else {
    //     value = "- -";
    ...
    //     value =
    //       this.data.positionDirection == 1
    //         ? (this.loss * 1 - this.data.positionAveragePrice * 1) *
    //           this.data.faceValue *
    //           this.data.positionAmount
    //         : (this.data.positionAveragePrice * 1 - this.loss * 1) *
    //           this.data.faceValue *
    //           this.data.positionAmount;
    //   } else {
    //     value = "- -";
  ...
  methods: {
    ...
    amountChange(e) {
      ...
      switch (this.quantityUnit) {
        ...
        case 2:
          ...
          // let res = checkUtils.accMul(e, countPrice);
          // e/(lastPrice * faceValue)
    ...
    profitChange(e) {
      ...
      if (/[!~@#$%&=]+/gi.test(this.numValue) && this.leverValue) {
        this.amount = Math.round(
          this.data.positionAmount * (this.leverValue / 100)
        );
      } else {
    ...
    handleNext() {
      ...
      data.lossType = this.sactiveindex2 == 1 ? 2 : 1;
      data.profitType = this.sactiveindex1 == 1 ? 2 : 1;
      ...
      //预计盈亏
      // console.log("prompt", this.data.positionDirection);
      // let p = this.data.positionDirection == 1 ? 1 : -1;

      // this.expectProfit =
      //   p * (data.triggerProfitPrice * 1 - this.data.positionAveragePrice * 1);
      // this.expectLoss =
      //   p * (data.triggerLossPrice * 1 - this.data.positionAveragePrice * 1);
      this.submitData = data; //提交的数据
    ...
    //参数初始化
    initParams() {
      this.numValue = null; //数量
      this.profit = null; //止盈触发价
      this.loss = null; //止损触发价
      this.limitProfitPrice = null; //止盈现价价格
      this.limitLossPrice = null; //止损现价价格
  ...
  watch: {
    ...
    isShow() {
      ...
      console.log("实时价格", this.priceObj.lastPrice);
    },
    data: {
      handler() {
        this.$nextTick(() => {
