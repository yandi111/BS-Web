  ...
  <div style=" background: #1B1B1B; border-radius: 10px;">
   <div style="padding: 40px; width: 480px;">
    <div>
     ...
     <div class="ff" style="margin-top: 36px; font-size: 14px;font-weight: 500; color: #737373; margin-bottom: 28px;">
      {{ titleShowRule }}
     </div>
    </div>

    <div v-for="option in getAuthOptions" :key="option.method">

     <div v-if="option.method === 'EMAIL'">
      <EmailCheck :option=option @emailCheckCodeClick='emailCheckCodeClick' :stateShow="stateShow"/>
     </div>

     <div v-if="option.method === 'PHONE'">
      <PhoneCheck :option=option @phoneCheckCodeClick='phoneCheckCodeClick' :stateShow="stateShow"/>
     </div>

     <GoogleCheck @gooleCheckCodeClick='gooleCheckCodeClick' :stateShow="stateShow"
                  v-if="option.method === 'GOOGLE_AUTHENTICATOR'"/>
    </div>
    ...
    v-model="verifyCode"
    @input="handleInput"
     @focus="eventFocus($event)" @blur="$event.target.style.border = 'none', eventFlag = false"
      class="custom-input"

      :placeholder=" phoneEmail ? '请输入4位手机验证码':'请输入4位邮箱验证码' "
      type="text" />
    ...
    <div style="display: flex; justify-content: center; align-items: center;height: 50px; margin-top: 40px;">
...
import EmailCheck from "@/views/VerificationCode/EmailCheck.vue";
import PhoneCheck from "@/views/VerificationCode/PhoneCheck.vue";
import GoogleCheck from "@/views/VerificationCode/GoogleCheck.vue";
...
export default {
 name: "FormVcode",
 components: {
  MobileCode, EmailCheck, PhoneCheck, GoogleCheck
 },
 data() {
  return {
   seconds: 60, // 倒计时数据
 ...
 computed: {
  ...
  promptNumber() {
   if (this.getAuthOptions && this.getAuthOptions[0]?.prompt) {
    console.log(this.getAuthOptions, 'getMethodgetMethodgetMethod000');
  ...
  // eslint-disable-next-line vue/return-in-computed-property
  titleShowRule() {
   if (this.getAuthOptions && this.getAuthOptions.length === 1) {
    if (this.getAuthOptions[0].method === 'GOOGLE_AUTHENTICATOR') {
     return '请输入谷歌验证码,验证码十分钟内有效'
    } else if (this.getAuthOptions[0].method === 'PHONE') {
     return `获取并输入手机 ${this.getAuthOptions[0].prompt} 收到的验证码，验证码10分钟有效 `
    } else {
     return `获取并输入邮箱 ${this.getAuthOptions[0].prompt} 收到的验证码，验证码10分钟有效 `
    }
   }
  ...
  stateShow() {
   return !(this.getAuthOptions && this.getAuthOptions.length === 1);
  }
 },
 mounted() {
 ...
 methods: {
  ...mapMutations(['setToken', 'setUid', 'setExpireTime', 'setNickname', 'setAvatar', 'setVipLevel', 'setAccountList', 'setUserList', 'setSppUserSecurity']),
  ...mapActions(['fetchUserInfo']),

  emailCheckCodeClick(emailCheckCode) {
   this.emailCheckCode = emailCheckCode
  },

  gooleCheckCodeClick(gooleCheckCode) {
   this.gooleCheckCode = gooleCheckCode
  },

  phoneCheckCodeClick(phoneCheckCode) {
   this.phoneCheckCode = phoneCheckCode
  },

  verificationCode() {
   console.log(8888);
   this.$refs.mobileCode.openDialog(this.getMethod)
  },
  ...
  registerBtn() {
   for (const item of this.getAuthOptions) {
   ...
   const authVerifyRequestVO = {
    ...
    verifyInfos: [
     ...
     {
      verifyCode: this.gooleCheckCode,
  ...
  registerFn() {
   Promise.try(async () => {
    return await forgetPassword({bizId: this.getBizId})
   }).then(res => {
     console.log(res,"forgetPassword=========>")
   })
  },
  ...
  phondeVerifyCode() {
   ...
   Promise.try(async () => {
    return onSendCode(params);
   }).then(() => {
    this.$customMessage(1, '发送成功')
