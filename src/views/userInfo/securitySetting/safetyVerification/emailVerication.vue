 ...
 <div class="phone-verication">
  ...
  <div class="form-container">
   ...
   >
    ...
    <el-form-item v-if="veridata &&veridata.authPhone" prop="phoneCode" :label="$t('userInfo.手机验证码')">
     <el-input
     ...
     >
            ...
            >
      <span class="code resend" slot="suffix" v-else>{{ $t("userInfo.重新发送") + codeTime + "s" }}</span>
     </el-input>
    </el-form-item>
...
import {resetEmailCheck} from "@/api/user";
 ...
 methods: {
  ...
  submit() {
   this.$refs.ruleForm.validate((valid) => {
    ...
    Promise.try(async () => {
     return await resetEmailCheck(params)
    }).then(res => {
     this.$emit('submit', res.data.successToken)
    })
   });
  },
 },
