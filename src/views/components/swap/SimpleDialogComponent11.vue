   ...
   <div v-if="isDialogVisible" class="dialog-overlay" @click.self="isDialogVisible = false">
    <div class="dialog-content" style="border:1px solid #252525;">
     ...
     <div class="flex jb"
      ...
      <div class="ff" style="font-size: 14px; color: #FFFFFF; ">一键反手</div>
      <div>
     ...
     <div class="con " style="padding-top: 10px;padding-left: 17px; padding-right: 17px;">
      ...
      <div
       ...
       <div style="height: 22px; line-height: 22px;"> 市价平仓当前仓位后，以同等数量反向市价开仓<br></div>
       <div style="height: 22px; line-height: 22px;"> 注:反手开仓功能仅对可用仓位生效，反手并不保<br></div>
       <div style="height: 22px; line-height: 22px;">证100%成功，如平仓后保证金不足等因素可能导<br></div>
       <div style="height: 22px; line-height: 22px;">致反向开仓的数量小于原来仓位的数量。</div>


      </div>

      <button @click="closeDialog" style="" class="confirm-button">确定</button>
...
export default {
 ...
 methods: {
  ...
  openDialog(active, token) {
   this.isDialogVisible = true
   this.backhandData = active
   this.tokenInfo = token

  },
  closeDialog() { //tokenInfo
