<template>
 <div style="margin-bottom: 29px;">
  ...
  <div>
   ...
   <div @click="$refs.mobileCode.openDialog('NEW_PHONE',phoneData)"
...
export default {
 ...
 methods: {
  handleInput() {
   this.$emit('phoneINPUTCode', this.phoneCheckCode)
  },
  ...
  phondeVerifyCode() {
   // this.setPhoneData(this.phoneData)
   this.getCode(this.bizId, this.authBizEnum, this.phoneData)
  },

  getCode(bizId, authBizEnum, phoneData) {
   if (!this.phoneData) return this.$customMessage(1, '手机号不能为空')
   Promise.try(() => {
    return onSendCode({bizId, method: this.method, authBizEnum, to: phoneData})
   }).then(() => {
