   ...
   <div v-if="isDialogVisible" class="dialog-overlay" @click.self="isDialogVisible = false">
    <div class="dialog-content" style="border:1px solid #252525;">
     <div class="flex jb"
      ...
      <div class="ff" style="font-size: 16px; color: #F0F0F0;  ">{{$t('更改谷歌验证')}}</div>
      <div>
     ...
     <div style="padding-left: 30px; padding-right: 30px; padding-top: 30px;">
      ...
      <div style="font-size: 12px; color: #737373; margin-top: 22px;" class="ff">
       <div style="color: #F0F0F0;font-size: 14px; font-weight: 500; margin-bottom: 19px;">1 {{$t('lang_38')}}</div>
       <!-- 手机验证码  -->
       ...
       <div style="font-size: 12px; color: #737373; margin-top: 30px;" class="ff">
        ...
        <div style="margin-top: 12px; color: #ED3C2F;">
         {{$t('请务必妥善保管谷歌验证秘钥，以免更换或丢失手机导致无法换绑。未保存密钥的情况下删除谷歌验证')}}
        </div>
       ...
       <div style="margin-top: 14px;" class="flex">
        <div style="height: 134px; margin-right: 15px;"><img class="img100"
                                                             src="@/assets/userInfo/Group171.png" alt=""></div>
        <div class="flex" style="margin-top: 101px;">
         <div style="color: #737373;">{{$t('秘钥')}}:{{ secretKey }}</div>
         <div @click="$copyContent(secretKey)" style="display: flex; margin-left: 2px; margin-right: 2px;">
          <div style="width: 20px;  cursor: pointer; margin-top: 1px; height: 20px;color: #F0F0F0">
           <img width="100%" height="100%" src="@/assets/side/icon_responsible.png" alt=""></div>
         </div>
        </div>
       </div>
      </div>

      <div style="place-self: 30px; padding-right: 30px; padding-top: 30px;">
       <div style="color: #F0F0F0;font-size: 14px; font-weight: 500;">3 {{$t('lang_2599')}}</div>

       <!-- 旧谷歌验证码-->
       <div style=" margin-top: 19px; width: 100%">
        <div class="ff0" style="font-size: 14px; margin-bottom: 9px; margin-top: 25px;">{{$t('旧谷歌验证码')}}</div>
        <div class="input-container"
         ...
         <div class="inputLeft" style="height: 40px; width: 100%;">
          <input maxlength="4" v-model="oldGoogleCode" @focus="eventFocus($event)"
                 @blur="eventFlags = false" class="custom-input" :placeholder="$t('请输入旧谷歌验证码')" type="text"
       ...
       <!--新谷歌验证码-->
       <div style=" margin-top: 19px; width: 100%">
        <div class="ff0" style="font-size: 14px; margin-bottom: 9px; margin-top: 25px;">{{$t('新谷歌验证码')}}</div>
        <div class="input-container"
             :style="{ border: eventFlagstal ? '0.5px solid #90FF00' : '0.5px solid rgba(0,0,0,0)' }"
             style="padding-right: 12px;">
         <div class="inputLeft" style="height: 40px; width: 100%;">
          <input maxlength="4" v-model="newGoogleCode" @focus="eventFocustal($event)"
                 @blur="eventFlagstal = false" class="custom-input" :placeholder="$t('请输入新谷歌验证码')" type="text"
                 style="padding-left: 12px; color: #F0F0F0;border:none;height: 40px;"/>
         </div>
...
import {onModifyGoogleBindKey, onModifyGoogleKey} from "@/api/user";
...
export default {
 ...
 methods: {
  ...
  check() {
   Promise.try(() => {
    return onModifyGoogleKey()
   }).then(res => {
    const {googleTotpKey} = res.data
  ...
  bindPhoneCodeSubmit(bizId) {
   Promise.try(() => {
    return onModifyGoogleBindKey(bizId)
   }).then(res => {
   if (res.code ==200) {
    this.fetchUserInfo();
