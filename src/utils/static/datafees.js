...
class datafeeds {
	 ...
	 */
	getBars(symbolInfo, resolution, rangeStartDate, rangeEndDate, onHistoryCallback, onDataCallback,  onErrorCallback) {
		

		const onLoadedCallback = data => {
	
			const isdata=	data && data.length
			isdata? onDataCallback(data, {
				noData: false
			}) : onDataCallback([], {
				noData: true
			})
		...
		this.self.getBars(symbolInfo,resolution , rangeStartDate, rangeEndDate, onHistoryCallback, onLoadedCallback,onErrorCallback)
	}
	...
	defaultSymbol() {
		return {
			...
			'supported_resolutions': ['1', '3', '5', '15', '30', '60', '120', '240', '360', '480', '720', '1D', '3D', '1W','1M'] //配置分时
		}
	}
}
