...
<template>
 <div style="width: 100%;" v-if="phoneState">
  <div class="flex ic jc"
       ...
       style="cursor: pointer;  border-radius: 4px; height: 57px; background: #252525;
   padding-left: 26px;
  color: #F0F0F0;">

   <div>
    <div style="display: flex;">
     <div class="flex ic jc" style=" height: 26px; border-radius: 4px; font-size: 12px;" @click="toggleDropdown"> +{{selectedOption}}
    ...
    <div v-if="isDropdownVisible" class="dropdown">
     <div style="overflow-y: scroll; height: 296px;
      ...
      <div style=" width: 100%;
       ...
       <div style="font-size: 12px;"><span>+{{ item.code }}</span></div>
      </div>
   ...
   <input v-model="phoneNumberData" maxlength="11" placeholder="请输入手机号码"
          @focus="blurstate = true, phoneCheck = false" @blur="$event.target.style.border = 'none', blurstate = false"
          style="color: #F0F0F0; text-align: left; font-weight: 600; outline: none;text-align: left;width: 100%; caret-color: #90FF00;margin-right: 4px;border: none; border-radius: 4px; background: #252525;  height: 55px; line-height: 57px;"
          type="text">
  </div>
  ...
  <div style="height: 57px;">
   <input v-model="tgDate" maxlength="10" @focus="$event.target.style.border = '0.5px solid #90FF00'"
          @blur="$event.target.style.border = '0.5px solid rgba(0,0,0,0)'"
          style="width: 100%; height: 57px;  caret-color: #737373; outline: none;font-size: 14px;font-weight: 500;
  ...
  <div style="display: flex; align-items: center;">
   <div @click="invitesFlag = !invitesFlag, isRotateds = !isRotateds;"
        style="cursor: pointer; font-size: 14px; color: #737373; margin-top: 26px;">邀请码(选填)
   </div>
   <div @click="invitesFlag = !invitesFlag, isRotateds = !isRotateds;" style="margin-top: 8px;">
    <img src="" alt="">
    <div style="margin-left: 8px; " class="triangle" :class="{ rotate: isRotateds }"></div>
   </div>
  </div>
  <input v-model="inviteData" maxlength="10" v-if="invitesFlag"
  ...
  <div style="font-size: 14px; color: #737373; margin-top: 20px;">注册用户即表示我同意 <span
   style="color: #B3B3B3;">《用户协议》</span> 和 <span style="color: #B3B3B3;">《隐私协议》</span></div>
  ...
  <div @click="$router.push('/login')" style="font-size: 14px; color: #F0F0F0; margin-top: 40px;">已有BSEX账号? <span style="color:#90FF00">去登录</span></div>
...
export default {
 ...
 methods: {
  ...
  handleMouseDown() {
   console.log("按钮被按下！", "handleMouseDown222");
   this.msState = true
   ...
   if (!isValid) {
    ...
    Promise.try(async () => {
     return await onRegisterCheck({
      ...
      inviteCode: this.inviteData,
      telegramId: this.tgDate,
      bizId: this.bizId
     })
    }).then(res => {
     const {authBizEnum, authOptions, bizId} = res.data

     this.setUsername(newPhoneNumberData)
